...
public class SearchDialog extends FocusDialog implements ActionListener, DocumentListener {
    ...
    public SearchDialog(MainFrame mainFrame) {
        ...
        compPanel.addRow(Translator.get("search_dialog.search_in"), searchInField, 10);

        searchSubfolders = new JCheckBox(Translator.get("search_dialog.search_subfolders"), true);
        compPanel.addRow("", searchSubfolders, 10);

        searchArchives = new JCheckBox(Translator.get("search_dialog.search_archive"));
        compPanel.addRow("", searchArchives, 10);

        searchHidden = new JCheckBox(Translator.get("search_dialog.search_hidden"));
        compPanel.addRow("", searchHidden, 10);

        matchCase = new JCheckBox(Translator.get("search_dialog.case_sensitive"));
        compPanel.addRow("", matchCase, 10);

        matchRegex = new JCheckBox(Translator.get("search_dialog.regular_expression"));
        compPanel.addRow("", matchRegex, 10);

        matchAll = new JCheckBox(Translator.get("search_dialog.match_entire"));
        compPanel.addRow("", matchAll, 10);
    ...
    private String getSearchQuery() {
        ...
        if (matchCase.isSelected())
            properties.add(new Pair<>(SearchBuilder.MATCH_CASE, Boolean.TRUE.toString()));
        if (matchRegex.isSelected())
            properties.add(new Pair<>(SearchBuilder.MATCH_REGEX, Boolean.TRUE.toString()));
        if (matchAll.isSelected())
            properties.add(new Pair<>(SearchBuilder.MATCH_ALL, Boolean.TRUE.toString()));
